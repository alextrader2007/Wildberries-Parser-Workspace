...
router.post("/", async (req, res) => {
  try {
    const { sellerId, pages, dest, curr } = req.body;
    ...
    if (!data.success) {
      return res.status(502).json({ error: data.error || "Ошибка поиска по продавцу через браузер." });
    }

    res.json({ success: true, products: data.products || [], count: data.count || 0, sellerId });
  } catch (e: any) {
