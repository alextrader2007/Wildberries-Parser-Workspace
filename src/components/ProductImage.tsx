...
async function findBasket(id: number): Promise<string> {
  ...
  if (await check(staticBasket)) { basketCache.set(vol, staticBasket); return staticBasket; }

  const common = ["39", "40", "41"].filter(b => b !== staticBasket);
  for (const b of common) {
    if (await check(b)) { basketCache.set(vol, b); return b; }
  }
  ...
  for (let i = 0; i < allBaskets.length; i += 10) {
    ...
    if (found) { basketCache.set(vol, found.b); return found.b; }
  }

  basketCache.set(vol, staticBasket);
  return staticBasket;
}

export default function ProductImage({ id, basket, alt, className }: { id: number; basket?: string; alt: string; className?: string }) {
  const vol = Math.floor(id / 100000);
  const [currentBasket, setCurrentBasket] = useState(() => basket || basketCache.get(vol) || getBasketStatic(vol));
