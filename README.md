...
## Быстрый старт (единственный способ для новичков)

### Автоматическая установка и запуск

Скопируйте проект на свой ПК и **просто дважды кликните `start.bat`** — он сделает всё сам:
...
### Ручная установка (только если start.bat не сработал)

```bash
# Сначала перейдите в папку проекта:
cd путь/к/Wildberries-Parser-Workspace-master
...
> ⚠️ Команды выше нужно выполнять в **cmd.exe** (не в PowerShell) из **папки проекта**, а не из `C:\WINDOWS\system32`.
